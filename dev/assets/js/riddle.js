$( document ).ready(function() {
	...
	if(localStorage.success && localStorage.error && localStorage.answers){
	...
	function checkAnswersSuccess(success){
		if(success == 1){
			...
			var an = parseInt(localStorage.getItem("answers"));
      var ax = an+1;
      localStorage.setItem("answers", ax);
			localStorage.setItem("success", 3);
			var code = Math.floor(Math.random() * 6000);
      ...
      console.log('games= '+localStorage.getItem("answers"));
		}
	}
	...
	function checkAnswersError(error){
		...
		if(error == 3){
			...
			var ad = parseInt(localStorage.getItem("answers"));
      var ac = ad+1;
      localStorage.setItem("answers", ac);
			$('.error-wrapper').show();
			$('#reload').show();
			$('#exit').hide();
			$('body').css('pointer-events', 'none');
	...
	'Poseedor de una pluma notable, fue una pieza clave para la consolidación de las instituciones educativas en México. En este segundo volumen de memorias narra sus experiencias con personajes ilustres y sus reflexiones sobre un amplio abanico de hechos que formaron el siglo XX.',
	...
	'Publicada en 1958 inmediatamente se volvió una de las obras más importantes en letras hispanoamericanas. Su título proviene de una expresión que podemos encontrar en Visión del Anáhuac de Alfonso Reyes, que a su vez recuerda la famosa frase de Alexander Von Humboldt. En esta novela se juega con la idea de la conformación de la ciudad a partir de la diversidad de orígenes, ideas y clases sociales. ',
	...
	'Textos y comentarios sobre estudios de la obra de Luis de Góngora, Reyes contempló varios aspectos de la vida del poeta, así como su influencia en otros escritores y su importancia para el mundo de las letras. Alfonso Reyes dedicó otros dos libros al mismo tema.'];
	var name_wt = ['Agustín Yáñez', 'Rubén Bonifaz Nuño', 'Alejando Rossi', 'Antonio Castro Leal', 'Christopher Domínguez', 'Enrique González', 'Fernando del Paso', 'Jaime García Terrés', 'Jaime Torres Bodet', 'Ramón Xirau', 'Salvador Elizondo', 'Vicente Quirarte', 'Juan Villoro', 'Carlos Fuentes', 'José Emilio Pacheco', 'Octavio Paz', 'Alfonso Reyes'];
	var book = ['Al filo del agua', 'Fuego de pobres', 'Manual del distraído', 'Díaz Mirón su vida y obra', 'Fray Servando', 'Babel', 'Noticias del imperio', '100 imágenes del mar', 'Memorias II', 'Naturalezas vivas', 'Farabeuf', 'La poética del hombre dividido en la obra de Luis Cernuda', 'El disparo de Argón', 'La región más transparente del aire', 'Las batallas en el desierto', 'Laberinto de la soledad', 'Cuestiones Gongorinas'];
	...
	$('#reload').on('click', function(){
		if (localStorage.error >= 3 || localStorage.success >= 3) {
      localStorage.setItem("success", 0);
      localStorage.setItem("error", 0);
      window.location.href = "index.html";
    }
